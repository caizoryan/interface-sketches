    ...
    <>
      ...
      >
        ...
        <p style="position:relative" class="boxtext">
          ...
          intellectual role of the designer." This is what my friend tells me,
          as we listen to each other’s anguished outpourings replete with VAT
          numbers, freelancing and short-term contracts. And that made me wonder
          what constitutes that role, whether it actually existed, how it
          vanished and what replaced it. Trying to answer these questions, I’d
          like to focus on graphic design as it is the field where I come from,
          and I believe it represents a paradigmatic case within the so-called
          creative industries. These days we hear a lot about the gig economy,
          the economy of little jobs, of chores. Platforms like Uber, AirBnB or
          Foodora are quickly becoming the direct intermediaries of any kind of
          service, a seemingly unavoidable development since they are prone to
          monopolization. Graphic design is no exception: online marketplaces
          like Fiverr and TaskRabbit offer access to graphic designers able to
          create a logo, a website, a poster.
        </p>
      </div>
      <div
        class="probox"
        style={`position:absolute; top:${props.topdiv}; left:${props.leftdiv}; `}
      >
        Graphic Design
      </div>
    </>
...
points.forEach((p: any) => {
  ...
  const wi = (Math.random() * width) / 6 + 50;
  const hi = (Math.random() * height) / 4 + 100;
  nodeCollection.push(
    <Node
      left={p.x + "px"}
      top={p.y + "px"}
      ...
      height={hi + "px"}
      topdiv={p.y - 10 + "px"}
      leftdiv={p.x + wi - Math.random() * 100 + "px"}
      // s={"rg255 134 19)b("}
      s={"rgb(255 255 0)"}
      // s={`rgba(${Math.random() * 255},${Math.random() * 255},${
      //   (Math.random() * 255, 1)
